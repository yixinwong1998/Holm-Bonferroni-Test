...
clear;clc
%% Experimental info
Dim = 50;  % dimension of benchmark
independentLength = 30;  % times of independent experiments

Algorithm1 = 'SBFOQS';  % Your algorithm 
Algorithm2 = 'GPSOWV';
Algorithm3 = 'FIPS';
Algorithm4 = 'CLPSO';
Algorithm5 = 'ALCPSO';
...
NP = 16; % the number of benchmarks
NA = 7;  % the number of benchmarks
...
for i = 1:NP
    % Fetch Data   
    for k = 1:NA
        filename = ['.\res\' eval(['Algorithm' int2str(k) ]) '_FN(' int2str(i) ')_D(' int2str(Dim) ')-Results.txt'];
        % Result: independentLength * NA
        Result(:,k)=load(filename)'; % read the results
    end
...
function [AlgsRank, CompR, zi, pi, Adjusted_Pvalue, Hypothesis] = holm_bonferroni(NP, NA, FinalAverOut)
    ...
    fprintf('  TABLE*** Holm-Bonferroni test result£¨reference£ºSBFOQS£¬Rank£º%.3f£©  \n', R0);
